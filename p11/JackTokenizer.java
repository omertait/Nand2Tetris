...
public class JackTokenizer {
    ...
    private ArrayList<String> tokens; // tokens in cur line
    ...
    // move one token back
    public void PrevTokenPos(){

        if (tokenPos > 0) {
            // if String const maybe need to move tokenPos several steps 
            if (tokenType() == tokenConst.STRING_CONST){
                tokenPos--;
                curToken = tokens.get(tokenPos);
        ...
        if (hasMoreTokens()){
            if (tokenPos >= tokens.size()){
                if(hasMoreLines()){
                    ...
                    tokens.addAll(Arrays.asList(curLine.trim().replaceAll(" +", " ").split(" ")));
                    curToken = tokens.get(tokenPos);
                    // if multiple words String const maybe needs to move several steps
